...
- 学校管理员：教师管理、学生管理、班级管理、任课关系、学科前测、共享题审核、共同题集合、跨校资源审核和学习数据检查。
...
- 数据能力：版本化学习事件、批量防重复接收、新旧记录兼容写入、历史记录转换、学生学习任务关联、评分版本、课堂积分流水、七项学习数据检查、评价冻结快照、共同题比较及日/7 日/30 日/单元学习情况汇总。

当前开发校最近完整 7 日数据检查已通过，旧事件未转换比例为 0%。教师端已完成评价标准制定、课时绑定、课堂冻结和材料关联；题库已支持普通题、共同题、分层题、题目版本和测试版本比较；教师可查看日、7 日、30 日和单元学习情况，并审核学生不可见的透明规则建议。正式机器学习模型、班级调整参数、正式学校试用和跨校验证仍在后续开发范围内。
...
Celery 当前用于每日 01:30 学习数据检查和 02:30 学习情况汇总。正式夜间任务排除模拟学校；班级模型训练、学校数据交换仍在后续开发范围内。
