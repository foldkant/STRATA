...
- 数据能力：35 个严格事件类型、不可修改的 `LearningEventV2`、批量防重复接收、新旧事件同步写入、历史记录转换、学生学习任务关联、评分版本、课堂积分流水，以及七项数据检查指标和夜间自动检查。
...
检查指标、判断标准和下一阶段条件见 [学习数据自动检查](docs/data_quality_pipeline.md)。
