...
class LearningWebPageAdmin(admin.ModelAdmin):
    list_display = ("title", "lesson", "teacher", "revision_no", "status", "is_active", "updated_at")
    list_filter = ("status", "is_active", "school")
    search_fields = ("title", "lesson__title", "course__title", "teacher__username", "teacher__display_name")
...
class LearningWebPageResponseAdmin(admin.ModelAdmin):
    list_display = ("page", "form_id", "student", "class_group", "attempt_no", "submitted_at")
    list_filter = ("form_id", "school")
    search_fields = ("page__title", "student__username", "student__display_name", "class_group__name")
...
class ClassroomGroupCollaborationAdmin(admin.ModelAdmin):
    list_display = ("session", "status", "grouping_strategy", "document_type", "storage_quota_mb", "updated_at")
    list_filter = ("status", "grouping_strategy", "document_type")
    search_fields = ("session__title", "session__course__title", "session__class_group__name")


@admin.register(ClassroomGroup)
class ClassroomGroupAdmin(admin.ModelAdmin):
    list_display = ("name", "collaboration", "group_no", "layer_hint", "leader", "updated_at")
    list_filter = ("layer_hint",)
    search_fields = ("name", "collaboration__session__title", "leader__username", "leader__display_name")
...
class ClassroomGroupFileAdmin(admin.ModelAdmin):
    list_display = ("original_name", "group", "uploader", "file_ext", "file_size", "created_at")
    list_filter = ("file_ext",)
    search_fields = ("original_name", "group__name", "uploader__username", "uploader__display_name")


@admin.register(ClassroomEvaluationConfig)
class ClassroomEvaluationConfigAdmin(admin.ModelAdmin):
    list_display = ("course", "enable_self", "enable_peer", "enable_teacher", "updated_at")
...
class ClassroomEvaluationSubmissionAdmin(admin.ModelAdmin):
    list_display = ("course", "class_group", "session", "evaluation_type", "evaluator", "target", "group", "updated_at")
