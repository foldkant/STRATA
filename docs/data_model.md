...
- 当前 app 已完成隐私权限、学习记录、学习任务关联、评分积分、新旧记录兼容写入、学习数据检查、评价版本冻结、共同题比较、四类学习情况汇总、教师审核规则版建议、版本化学习指标、计划分析时间点、未来结果、冻结数据版本、重复测量统计、M00-M03 透明基线、CatBoost/LightGBM 同折比较和班级校准候选。历史旧记录使用确定性 UUID 回填，不能明确转换的记录以内部状态 `legacy.unmapped` 隔离，界面统一显示“旧事件未转换”。当前模型结果只完成模拟数据工程验收并用于影子核查，尚未通过正式学校真实纵向数据验证，不能作为自动教学决策或学生可见层级。
