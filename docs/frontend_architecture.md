...
- 教师 `/teacher/stratification` 页面包含“学习情况”和“分层建议”两个页签。学习情况支持当日、近 7 日、近 30 日和单元筛选及 XLSX 导出；建议仅教师可见。
- 教师审核支持采纳、保持、调整和暂缓。前端不把操作描述为自动生效，学生端也不建立对应层级页面。
- 桌面端表格允许容器内横向滚动；手机端筛选和主操作纵向排列，按钮保持至少 44px 可点击高度。
