...
- 复用 `AppShell`、`NoticeLine` 和 `EChartPanel`；页面只实现质量领域的指标卡、问题列表和运行阶段。
- 当前报告和七项阈值使用文字、数字和颜色共同表达；图表下方的运行记录与指标卡提供非图形信息兜底。
...
- 路由：`/teacher/measurement-design`，API 与类型集中在 `frontend/src/api/measurement.ts`。
- 页面复用 `AppShell`、`NoticeLine` 和 `ConfirmDialog`；蓝图、量规、条目编辑分别拆为组件，避免把完整编辑逻辑堆入主页面。
- 蓝图与条目使用三步弹窗，标题和底部操作固定，表单正文独立滚动；量规弹窗只管理总体信息和条目列表，进入条目编辑时隐藏量规弹窗。
- 桌面使用紧凑表格；小于 640px 时转为纵向信息块，不产生页面级横向滚动。
- 所有字段有可见标签，必填星号使用危险色；异步保存和发布均提供禁用状态、成功或字段错误反馈。

2026-07-19 使用 UI/UX 审查流程检查 `320x740`、`390x844`、`768x900` 和 `1440x900`：根页面横向溢出均为 0；蓝图、量规和量规条目弹窗正常打开，控制台无错误；临时截图审查后删除。
