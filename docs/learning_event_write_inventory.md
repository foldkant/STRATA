...
> 当前状态：`DATA-01C` 进行中；测试、课堂积分/聊天、课堂环节作答/附件、AI 学习网页、课堂普通资源、五星评价、小组协作、签到考勤、抢答响应和随机点名选择已切换，剩余入口见下表。
...
2. 尚未迁移的普通页面和部分课堂控制仍存在分散的 `LearningEvent.objects.create()`。
...
`DATA-01C` 改造时保留这个业务辅助函数作为过渡适配层，但内部只能调用统一领域服务，不能继续直接操作 V1 模型。
...
尚未迁移：资源中心自主浏览的指派语义、普通页面事件以及部分课堂控制审计事件。完成这些入口及质量对账前，不能把 V2 覆盖率表述为完整。
