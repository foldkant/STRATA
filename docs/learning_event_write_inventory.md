...
> 盘点日期：2026-07-18。  
> 用途：为旧业务 `LearningEvent` 到新版 `LearningEventV2` 的兼容写入、核对和回滚提供完整入口清单。  
> 当前状态：统一学习记录写入已经完成；生产写入全部收口到统一服务，历史回填和全库核对已通过。
...
迁移时统一调用 `learning_analytics.services.event_ingestion.record_learning_event()`。视图不得自行分别创建旧业务记录和新版记录。

## 2. 直接写入点

| 优先级 | 业务事实 | 当前入口 | 旧记录含义 | 新版记录目标 |
| --- | --- | --- | --- | --- |
...
| P0 | 课堂环节作答 | [`api/views.py`](../api/views.py) | 旧版 `_write_student_event(answer_submit)` | **已迁移**：`LessonStepAttempt/Answer` 保存正文与版本，逐题写入新版 `item.submitted@1.1` 和自动/待评记录 |
...
| P2 | 抢答响应与加减分 | [`api/views.py`](../api/views.py) | `page_view/teacher_intervention` | **已迁移**：可选 `interaction` 学习任务、服务端排名的 `quick_answer.responded`、积分流水及冲正；回答正文只留旧业务元数据 |
...
| P4 | 小组共享文件 | [`api/views.py`](../api/views.py) | 曾通过 `_write_student_event(task_submit)` 写文件名 | **已迁移**：组员定向可选任务和 `group.file.shared`；新版记录只保存文件 UUID、版本、格式和大小 |
...
- 部分教师加减分把学生保存为 `actor`，无法区分“教师执行动作”和“记录归属学生”。新版记录必须分别写 `actor` 与 `target_student`。
- 多种业务依靠 `metadata.action` 区分，事件名过粗；新版记录按登记后的点分事件名拆分。
- 旧课堂作答曾把答案和附件复制进事件 JSON；现已迁移到业务提交表，新版记录只保存业务引用、题目版本、学习任务和尝试 UUID。
- 未提交、未评分和未获得学习任务容易被解释为 0；必须先建立学习任务和最终评分记录，之后才能计算比率。
- 课堂积分直接覆盖 `StudentProfile.score`，缺少冲正流水；迁移后以不可变积分台账重算缓存。
- 客户端离线、迟到和网络故障尚未记录，现有“零行为”不能直接解释为低参与。

## 5. 迁移验收

1. 每个入口先生成稳定 `event_id`，统一服务在同一事务内完成新旧记录兼容写入。
2. 核对按业务事实和明确转换规则进行，不只比较总行数。
3. 主观题未完成评分前不得生成最终 0 分事实。
4. 结果事件必须关联明确学习任务；关联不足时只报告检查问题。
5. 兼容写入失败要整体回滚或进入明确重试队列，不能静默丢弃任一记录。
6. 停止旧记录写入前保留回滚开关，并完成重复、乱序、离线和迟到回放测试。

## 6. 统一写入当前进度

已迁移：

- 随机点名和抢答评分：旧页面继续读取旧业务记录，新版记录明确保存教师 `actor` 与学生 `target_student`，实际余额变化进入不可变积分流水。
- 聊天放行、警告、撤回和扣分：审核状态、新旧干预记录与积分流水处于同一事务；积分不足时不提交审核结果。
- 已提供 `dual_required/v1_only` 回滚模式和 `reconcile_learning_event_writes --check` 核对命令。
...
- 抢答正文只留在旧业务兼容元数据，新版记录不复制原文。随机点名写服务端来源的 `random_call.selected`，保存选择方法、候选人数和序次；该事实不生成学生学习任务，也不视为学生参与、完成或能力证据。

最终迁移：资源中心自主浏览使用 `resource.center.opened` 且不生成必做任务分母；普通课时过程、聊天、干预确认和课堂控制使用独立事件模式。开发库回填 606 条旧记录：7 条原已写入新版记录、179 条确定转换、420 条内部兼容状态 `legacy.unmapped`、0 条拒绝；重跑 606 条全部幂等命中。
