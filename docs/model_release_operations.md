...
发布在数据库事务中完成。签名、文件摘要或候选检查任一失败时，不会停用当前版本；失败原因写入不可修改的发布审计记录。仅生成候选不会提前隐藏当前已发布版本的教师待办；只有发布事务全部成功后，系统才把同范围旧待办转为历史并启用新待办。
...
回滚不重新训练模型，也不删除任何历史版本。回滚成功时同步恢复目标版本中尚未由教师处理的待办，并把被回滚版本仍待处理的建议转为历史；教师已经采纳、保持、调整或暂缓的记录不被改写。模型包丢失、摘要不一致、签名无效或学校/学科不一致时必须停止回滚。
