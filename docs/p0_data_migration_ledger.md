...
| 内容处理 | 逐页记录、内容条目和检索片段按版本保存；2026-07-22 12:40 验收快照中 34 个版本已完成文本处理、14 个扫描版本由低优先级独立后台队列逐个处理 |
| 检索索引 | 同一快照为 34 个版本索引、4,411 个可追溯检索片段；发布版本严格审计为 2 个版本、0 错误、0 警告 |
...
迁移后的 `manage.py check`、`makemigrations --check --dry-run` 和 `PRAGMA quick_check` 均通过；补齐 P1 生命周期与结构化 JSON 验收后，联合后端回归 90/90 通过。没有待执行的 P0 数据库迁移。
