...
| `DATA-01A` 事件注册表与新版事件 | 完成 | 已建立 35 个 Pydantic 严格事件模式、不可覆盖的 `EventSchemaDefinition`、不可变 `LearningEventV2`、同步命令、迁移和测试 |
...
| `DATA-02B` 评分成熟状态与积分流水 | 完成 | 已建立不可变评分版本、待评/最终/修订成熟状态、同尝试校验、积分幂等/冲正/非负余额和旧分数缓存核对服务 |
| `DATA-01C` 统一写入与新旧记录兼容 | 完成 | 所有旧版生产写入已收口到统一服务；确定性历史回填、无法转换旧记录隔离、幂等重跑和全库核对通过 |
| `INCENTIVE-01` 积分奖章边界 | 积分底座完成 | 积分与成绩、评价标准、核心素养和 AI 建议分离；奖章仍在评价标准和证据框架完成后开发，不提前进入主模型 |
| M0 总体 | 工程检查完成 | SQLite/PostgreSQL 全量测试、Redis/Channels、Celery worker/beat 和实际任务已验证；学校生产部署仍需完成密码、服务账号和备份恢复验收 |
| `DATA-03A/B` 学习数据检查 | 工程完成 | 七项指标、后续流程检查、本校 API/XLSX、夜间 Celery、失败重试和学校管理员页面已完成 |
| `SIM-01` 模拟数据开发 | 完成 | 独立模拟学校/校内测试、可重复纵向生成、模拟参考值、正式/测试数据隔离、整批清理和批次检查报告已完成 |
| M1 正式数据轨道 | 工程完成、检查通过 | 开发校最近完整 7 日为 10 条正式事件，旧事件未转换比例 0%，新旧记录差异 0%；数据量仍不足以形成真实模型结论 |
| M2 评价与题目准备 | 已进入、部分完成 | `EVAL-01A` 评价管理工程已完成；正式试用、专家审核、评分一致性、题目生命周期和共同测试规则仍未完成 |
...
不允许跳过学习记录、评价和题目阶段直接训练生产模型。开发库中的模拟数据只能用于接口、迁移、重算、恢复能力和故障测试，不能用于报告真实模型效果。正式学校数据与模拟数据必须分别通过各自检查，不能用模拟批次报告改变正式学校结果。任何里程碑只有在“代码、测试、文档、迁移、回滚和学习数据检查”同时完成后才能关闭。
...
| 领域 | 当前状态 | 路线要求 |
| --- | --- | --- |
| 学习事件 | [learning/models.py](../learning/models.py) 中 `LearningEvent` 使用粗粒度类型和自由 `metadata` | 新增版本化事件记录、事件注册表、学习任务分母、防重复和迟到规则；完成新旧记录兼容写入后逐步冻结旧表 |
...
保留现有 `learning` 业务模型，不做一次性大搬迁。`LearningEvent`、`StudentFeatureSnapshot` 和 `StratificationDecision` 作为旧业务兼容源，使用新增表完成新旧记录兼容写入、核对和渐进切换。`aiops` 只负责模型实验、版本、校准、签名、部署和监测，不负责课堂业务写入。
...
| M1 学习记录层 | 2-3 Sprint | 新版事件、学习任务、评分/积分/干预记录、兼容写入和检查报告 | M0 | 可靠描述“发生了什么、学生实际获得了什么学习任务” |
| M2 测量与题目层 | 2-3 Sprint | 评价方案、评价标准版本、AI 题生命周期、共同锚测 | M1 | 形成可验证的任务和测量数据 |
...
| M5 离线模型实验 | 3-5 Sprint，且等待真实结局成熟 | M00-M06 比较、外层验证、模型卡和候选结论 | M3 数据检查 | 选择是否存在优于透明基线的候选模型 |
...
- `DATA-01A` 已完成：迁移 `learning_analytics.0002` 新增事件模式和新版学习事件表。
...
- 积分服务已统一单次增减上限、非负余额、来源事件防重复、反向冲正和 `StudentProfile.score` 缓存核对；抢答、随机点名和聊天扣分已通过统一服务接入，旧历史分数仍不能直接当作完整研究流水。
...
- `DATA-01C` 首批新增迁移 `learning_analytics.0006`，以 `LearningEventV2.legacy_event` 建立一对一追溯。统一服务在同一事务内写入旧业务记录和新版记录，任一失败整体回滚；`LEARNING_EVENT_WRITE_MODE=v1_only` 仅用于紧急业务回滚。
- `reconcile_learning_event_writes --check` 可按全平台或学校分别检查实时兼容写入、历史确定转换、历史隔离和未关联旧记录。`backfill_learning_event_v2` 支持学校、截止时间、批次、dry-run 和断点恢复。
...
- 迁移 `learning.0008` 新增不可变追加式 `LessonStepAttempt`、`LessonStepAttemptAnswer`，并为 `StudentWorkAttachment` 增加提交 UUID、上传版本和前序版本引用。课堂答案正文和附件留在业务表，新版记录不复制原文或文件地址。
...
- 迁移 `courses.0018` 为每次 `LearningWebPageResponse` 生成唯一分析尝试 UUID。页面随环节投放生成学习任务；区块可见只采集页面版本、区块 ID/类型、可见时长和比例，表单答案不进入新版记录。
...
- `ClassroomEvaluationSubmission` 改为追加式版本：每次修订生成新的提交 UUID、递增 `submission_version`、`supersedes` 和分析尝试 UUID。`evaluation.rating.submitted` 只保存评价标准版本、评价项 ID 和 1-5 星，不复制评价备注。
- 课堂开启评价时按自评、互评、师评生成非必做任务机会；自评机会归本人，师评和互评机会归被评价学生。互评跨学生目标只允许经过已校验同组关系的服务端入口，学生批量事件 API 仍禁止替他人提交。
- 关闭评价或结束课堂会撤回未提交评价机会，已提交版本保留；课程级师评按课程、班级和评价标准版本建立独立机会。
...
- `attendance.recorded` 使用 `revision_no` 和 `supersedes_event_id` 保留状态变化；备注不进入新版记录。课堂结束时未响应任务撤回，但不自动标记缺勤，网络或设备问题不能被静默解释为负面行为。
...
- 关闭抢答或结束课堂时只撤回未响应任务。回答正文留在业务兼容层，不复制进新版记录；`random_call.selected` 只记录教师选择事实和序次，不创建学生完成任务，也不能直接作为掌握度或投入特征。
- 目前 `content.released` 只表示立即开放。未来定时开放不能提前写成 `released`，必须在后续工作包增加 `content.assigned` 后再实现。
- `DATA-01C/03` 已完成。新版记录可以进入后续特征工程，但在 M2 评价与题目检查、M3 特征定义和真实样本条件未完成前，不得生成或宣称真实模型性能。
...
所有业务写入调用一个 `record_learning_event()` 领域服务。新旧记录兼容写入由该服务负责，视图不能直接复制两次 `objects.create()`。

### API 与任务

- `POST /api/v1/learning-events/batch/`：批量补传、逐条幂等结果。
- `GET /api/v1/school-admin/analytics/quality/`：本校事件覆盖、重复、迟到和错误。
- Celery：`validate_events`、`build_opportunities`、`finalize_result_facts`、`compare_old_new_records`、`save_data_check_report`。

### M1 检查

- 设计报告中的黄金事件、离线乱序、迟到、机会分母、最终评分、评价归属和积分冲正测试通过。
- 每个结果都能追溯到投放机会、版本和最终状态；无机会不记失败，未评分不记 0 分。
- 新旧记录差异有明确原因和审计，核对未通过时保持兼容写入且不进入 M3。
- 核心数据源学习任务关联率达到当前工程判断标准；不足时只显示待处理问题。

## 6. M2：测量、评价标准与题目层

### 当前状态

`SIM-01` 已通过，评价管理工程实现已完成。正式学校数据检查现为绿色，但专家内容审核、真实试用、评分一致性、题目参数和跨校比较仍未完成。详见[模拟数据开发说明](synthetic_data_research_track.md)。

### 开发任务

1. 建立 `EvaluationPlanVersion`，保存课程目标、主张、证据、学习任务、思维要求、允许支持和验证用途。
2. 建立模块化 `EvaluationStandardVersion`、`EvaluationCriterionVersion`、星级表现说明、评分示例和材料引用；课程使用、校级分析和专项分析用途隔离。
3. 自评、互评、师评保存逐项星级、目标对象、评价标准版本和证据，不只存总星数。
...
6. 学校管理员维护评价标准；教师端增加题目验证状态和共同测试管理；学生端只显示当前任务需要的星级表现说明和本人任务。

### M2 检查

- 学科专家完成内容审查，反应过程和第一轮试评方案已执行或正式排期。
- 未达到 `calibrated/active` 的 AI 题无法进入共同锚测、主要研究结局或跨校链接。
- 五星评价逐项带可观察的星级表现说明，未启用项目保存 `暂不评价` 而不是 0/1 星。
...
- 旧 `current_layer` 与 `StudentSubjectBand` 并行读取核对完成，新的个体投放只读取版本化有效带。
...
- 候选模型在锁定外层验证中稳定优于透明基线，且校准、公平、解释稳定和拒绝预测满足预注册检查。
...
1. **新增结构**：只新增新版表、索引和约束，不修改旧表含义。
2. **兼容写入**：统一事件服务同时写旧业务记录和新版记录，失败时事务回滚或进入明确重试队列。
3. **历史回填**：只迁移能够明确转换的历史记录；不确定记录保留为内部隔离状态。
4. **记录核对**：按事件、学习任务、结果和积分分别核对，不能只比总行数。
5. **切换读取**：学习数据检查先读新版记录，随后是特征、教师工作台和学生投放。
6. **冻结旧表**：所有读写切换并稳定一个批准周期后，旧表改为只读。
7. **清理旧结构**：只有备份、申诉、研究复现和保留要求均满足后才删除旧字段或表。
...
| M1 | 事件写入无感接入 | 离线批量补传 | 学习数据检查 | 无 |
| M2 | 评价标准/题目生命周期 | 星级表现说明与评价 | 评价版本概况 | 无 |
| M3 | 特征检查页 | 无分层变化 | 学习任务/特征检查 | 无 |
...
2. Issue 使用工作包前缀，例如 `DATA-01`、`EVAL-01`、`UX-01`、`MODEL-01`。
...
8. `DATA-01C`：统一事件写入服务与新旧记录兼容写入。**已完成；生产直写清零，历史回填与核对通过。**
9. `DATA-03A`：学习数据检查、新旧记录核对和本校检查 API。**已完成。**
10. `DATA-03B`：学校管理员学习数据检查页面与 Celery 夜间调度。**已完成；当前正式学校最近 7 日检查为绿色。**
11. `SIM-01`：独立模拟学校、可重复纵向数据、模拟参考值和正式统计隔离。**已完成；只用于模拟工程开发。**
12. `EVAL-01A`：一个试点学科的评价方案和评价标准版本。**已完成模拟数据环境下的工程实现；真实效度验证未开始。**
13. `EVAL-01B`：内容专家审查、反应过程、评分者培训和共同锚测记录契约。**下一工作包。**
14. `ITEM-01A`：AI/人工题目生命周期、曝光、审核、试测和退役状态。
15. `LINK-01A`：共同锚题、跨版本等值和评分一致性分析。
16. `FEAT-01A`：M2 评价与题目检查通过后，建立特征定义注册表和首个无模型快照。

完成前 10 项以前不开发真实模型训练按钮；完成 M3 前，教师分层页面只使用模拟 fixture 做组件开发，不读取开发库生成伪 AI 结果。

## 18. 全局完成定义

一个“AI 隐性动态分层版本”只有同时满足以下条件才可称为完成：

- 数据事实、机会、测量、特征和计划结局可重算并通过数据检查。
