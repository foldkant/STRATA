...
| 工作项 | 状态 | 当前结果 |
| --- | --- | --- |
| `ARCH-01` analytics 领域骨架 | 进行中 | 已创建 `learning_analytics` app、独立 `api/analytics/` 路由包和测试目录；前端教师工作台 feature 在 M4 建立 |
| `PRIV-01A` 学生字段泄漏审计 | 首批完成 | 已覆盖学生资料、课堂、分层题、小组、互评、聊天组名和直接对象访问；后续每新增学生接口继续执行字段白名单测试 |
...
| M0 总体 | 进行中 | 事件写入点清单和 M1 迁移准备已完成；尚需正式 PostgreSQL/Redis 环境验收 |
