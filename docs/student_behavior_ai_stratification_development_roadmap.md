...
| `DATA-03A/B` 数据质量 | 工程完成 | 七项指标、强制闸门、本校 API/XLSX、夜间 Celery、失败重试和学校看板已完成 |
| `SIM-01` 合成数据研究轨道 | 完成 | 独立学校/校内叠加、确定性纵向生成、隐藏真值、质量范围隔离、整批清理和绿色合成报告已完成 |
| M1 正式数据轨道 | 工程完成、数据阻断 | 开发校最新报告语义缺失率 37.5%，闸门为红色；不得据此进入真实测量或模型结论 |
| M2 合成工程轨道 | 待开始 | 允许使用合成批次开发量规、题目生命周期和共同锚测契约；专家验证和真实试评仍未开始 |
...
不允许跳过数据与测量阶段直接训练生产模型。开发库中的合成数据只能用于接口、迁移、重算、恢复能力和故障测试，不能用于报告真实模型效果。正式学校数据与合成研究数据必须分别通过各自闸门，不能用合成报告把正式学校报告变绿。任何里程碑只有在“代码、测试、文档、迁移、回滚和数据质量”同时完成后才能关闭。
...
`SIM-01` 已通过，允许开始 M2 的合成工程实现。正式学校轨道仍为红色，因此专家内容效度、真实反应过程、评分一致性、题目参数和跨校等值结论均保持未完成。详见[合成数据研究轨道](synthetic_data_research_track.md)。
...
9. `DATA-03A`：质量流水线、对账和本校质量 API。**已完成。**
10. `DATA-03B`：学校管理员质量页面与 Celery 夜间调度。**已完成；当前正式学校数据闸门仍为红色，暂不进入真实 M2 验证。**
11. `SIM-01`：独立合成学校、确定性纵向数据、隐藏真值和正式统计隔离。**已完成；只开放合成工程轨道。**
12. `MEAS-01A`：一个试点学科的任务蓝图和量规版本。**已完成合成工程实现；真实效度验证未开始。**
13. `MEAS-01B`：内容专家审查、反应过程、评分者培训和共同锚测记录契约。**下一工作包。**
14. `ITEM-01A`：AI/人工题目生命周期、曝光、审核、试测和退役状态。
15. `LINK-01A`：共同锚题、跨版本等值和评分一致性分析。
16. `FEAT-01A`：M2 测量闸门通过后，建立特征定义注册表和首个无模型快照。
