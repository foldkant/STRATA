...
| `SIM-01` 合成数据研究轨道 | 完成 | 独立学校/校内叠加、确定性纵向生成、隐藏真值、质量范围隔离、整批清理和绿色合成报告已完成 |
| M1 正式数据轨道 | 工程完成、检查通过 | 开发校最近完整 7 日为 10 条正式事件，旧事件未转换比例 0%，新旧记录差异 0%；数据量仍不足以形成真实模型结论 |
| M2 合成工程轨道 | 待开始 | 允许使用合成批次开发评价标准、题目生命周期和共同锚测契约；专家验证和真实试评仍未开始 |
...
不允许跳过数据与测量阶段直接训练生产模型。开发库中的合成数据只能用于接口、迁移、重算、恢复能力和故障测试，不能用于报告真实模型效果。正式学校数据与合成研究数据必须分别通过各自检查，不能用合成报告把正式学校报告变绿。任何里程碑只有在“代码、测试、文档、迁移、回滚和数据质量”同时完成后才能关闭。
...
2. 建立模块化 `EvaluationStandardVersion`、`EvaluationCriterionVersion`、锚点样例和证据引用；课程使用、校级分析和专项分析用途隔离。
...
6. 教师端增加评价标准设计、题目验证状态和共同锚测管理；学生端仍只显示评价锚点和本人任务。

### M2 检查

- 学科专家完成内容审查，反应过程和第一轮试评方案已执行或正式排期。
- 未达到 `calibrated/active` 的 AI 题无法进入共同锚测、主要研究结局或跨校链接。
- 五星评价逐项带可观察锚点，未启用项目保存 `暂不评价` 而不是 0/1 星。
...
| M2 | 评价标准/题目生命周期 | 任务锚点与评价 | 测量版本概况 | 无 |
...
9. `DATA-03A`：质量自动流程、对账和本校质量 API。**已完成。**
10. `DATA-03B`：学校管理员学习数据检查页面与 Celery 夜间调度。**已完成；当前正式学校最近 7 日检查为绿色。**
11. `SIM-01`：独立合成学校、确定性纵向数据、隐藏真值和正式统计隔离。**已完成；只开放合成工程轨道。**
12. `EVAL-01A`：一个试点学科的评价方案和评价标准版本。**已完成合成工程实现；真实效度验证未开始。**
13. `EVAL-01B`：内容专家审查、反应过程、评分者培训和共同锚测记录契约。**下一工作包。**
14. `ITEM-01A`：AI/人工题目生命周期、曝光、审核、试测和退役状态。
15. `LINK-01A`：共同锚题、跨版本等值和评分一致性分析。
16. `FEAT-01A`：M2 测量检查通过后，建立特征定义注册表和首个无模型快照。
