...
- 当前投放环节含分层题时，学生端只接收后端按本人当前层级过滤后的题目，不展示其他层级题目和分值。
- 学生端只标注“当前环节已匹配分层题”或“已按你的层级匹配”，不向学生展示模型解释、其他层级任务或分层后台原因。
...
- AI 分组只影响教师确认后的分组结果。
- 学生看到的是普通小组合作体验，不显示“AI 把你分到某组”的解释。
...
- 档案不向学生展示当前分层、模型置信度、风险标签、排名、总积分或教师内部干预详情。
