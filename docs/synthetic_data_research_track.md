...
平台尚无足以支持测量验证和模型研究的真实纵向数据。合成数据轨道用于提前验证事件、机会、评分、质量、特征和模型自动流程的工程正确性，不用于证明真实学生上的预测性能或教学效果。
...
- 正式试点前必须重新完成真实数据质量、评价标准、共同测量、样本量和伦理治理验收。
...
生成过程调用正式 `record_learning_event()`，同时产生 V1/V2、机会、状态转移、评分事实和接收计数，不绕过生产契约直接批量插表。
...
合成轨道通过后可开始 `EVAL-01A` 的数据库、API、版本冻结和测试实现，但仍需区分：
