...
- 开启或关闭本次课堂小组合作。
- 设置每组人数，范围 2-12 人。
- 设置分组策略：同层优先、严格同层、随机、AI 分层分组预留。
- 设置每组协作文档类型：Word、PPT、Excel。
- 设置每组共享空间配额，范围 10-2048MB。
- 设置是否允许学生在线编辑协作文档。
- 设置是否允许学生上传小组共享文件。
- 查看所有小组、成员、组长、层级提示、共享文件和空间使用情况。
- 打开任意小组协作文档进行查看或编辑。

默认分组规则：

- 第一版没有直接调用 AI 模型。
- `balanced_layer`：优先让 A/B/C 同层学生在同一组，未分层学生按现有组均衡补齐。
- `same_layer`：严格按 A/B/C 和未分层分别切组。
- `random`：按课堂 ID 做确定性随机分组。
- `ai_layer`：字段和入口已预留，当前回退到同层优先规则；后续接入模型后输出候选分组，仍由教师确认。
...
- 输入特征包括学生当前层级、前测结果、近 7/30 天学习事件、课堂参与、作答达成率、项目/任务提交质量、协作贡献和教师手动调整记录。
- 分组结果必须可解释，例如“同层支架”“同层挑战”“临时帮扶”“协作稳定性”。
- AI 只生成候选分组，教师确认后生效。
- 小组成员的协作文档编辑、文件上传、组内任务完成和教师评价会反哺后续分组模型。
...
- 评价结果不直接作为分层 label；教师最终确认的层级、教师采纳/拒绝 AI 建议和后续阶段表现才是更稳定的训练标签。
