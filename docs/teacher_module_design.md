...
- 项目式学习：适合跨课时、跨周期综合项目，以“项目 -> 阶段 -> 里程碑 -> 小组协作 -> 成果迭代 -> 评价标准评价”为主线。
...
- 任务驱动课程默认进入课时学习过程设计。
- 项目式学习课程默认进入项目设计器，项目设计器需要支持阶段、里程碑、甘特图、项目日志和评价标准。
...
- 分层分值是题目难度和评价设计上下文，不作为主要 label。教师最终确认层级和采纳/拒绝属于弱监督与实施数据；主要预测结局使用预先计划的下一阶段掌握、必做任务完成、逾期和成长结果。
...
- 签到开启后为全班建立独立考勤机会。教师可把学生状态追加修订为已签到、迟到、请假或缺勤；课堂结束后未响应保持未知，不自动判缺勤。
...
当前页面必须先选择本人一门课程，不提供跨学科“全部课程”汇总，避免把同一学生在不同学科的内容层级混成一行。完成/逾期支持、共同掌握内容建议、学习趋势和数据状态分开解释。
...
- 教师在“评价标准”页面制定、试用和发布标准，评价方式固定为 1-5 星或暂不评价。
- 课时设计只选择已发布标准，并选择自评、互评和师评；不重复维护评价项。
- 只有课堂开启小组合作并生成小组后，学生端才显示互评入口。
- 每次新建课堂默认不开放评价；课堂控制台只负责开启、关闭、执行师评和查看结果，不编辑标准或生成评价项。
- 评价结果写入评价提交表，并同步写入 `LearningEvent`。只有评价标准版本、材料关联、评分一致性和适用目的通过检查后，才可作为掌握或成长的候选证据；它不直接成为 A/B/C 标签，也不自动决定小组分组。
- 课堂首次开启评价时冻结标准版本、三类开关、完整指标、星级说明和内容哈希；后续工作稿或新版本只影响新课堂。
- 教师修改师评时追加新提交版本，不覆盖旧评价。自评、互评、师评统一保留评价者、被评价学生、评价版本和前序修订关系。
- 每个评价项可选择 1-5 星或“暂不评价”；暂不评价必须选择结构化原因，选择“其他”时填写具体说明。
- 评价备注和暂不评价补充说明只留业务表；分析事件保存评价项 ID、1-5 星和暂不评价原因代码。评价星级是任务内过程证据，不直接作为分层标签。
- 平均星级排除暂不评价项；教师端同时显示提交人数、已评分指标数和暂不评价指标数。
- 旧课程级评价入口返回 `410`，旧配置和旧版本只保留历史兼容读取。
...
- 原教师个人资源库升级为 `/teacher/resources` 教学资源中心。
- 教师可创建文件、图文、外部链接和学生项目四类资源。
- 发布范围支持仅自己、指定任教班级、本校共享和申请跨校共享。
- 跨校资源由学校管理员审核；退回原因在教师资源卡片中显示。
- 学生项目支持个人或小组成员、所属课程、比赛和获奖信息；日志、甘特图、阶段成果等过程材料全部选填。
- 课时设计器资源面板可切换个人、校内和跨校资源，并复用同一资源绑定校验。

完整规则见 `docs/resource_center_design.md`。

## 22. 制定和使用评价标准

- 教师在 `/teacher/evaluations` 为本人课程制定评价方案和评价标准，并发布不可直接修改的历史版本。
- 教师在课时设计的“环节评价”中选择本人课程已发布的评价标准，并为该环节启用自评、互评和教师评价。
- 课堂首次开启评价时冻结环节绑定、标准版本、完整指标和五级星级说明；已投入课堂的绑定不能原地修改或删除。
- 评价提交自动关联同一被评价学生在本课堂、本环节的最新作答和上传作品，便于教师按真实材料评价。
- 教师可以在课堂收尾时开启或关闭评价入口；已发布版本不能直接修改，需要修改时生成下一版本并用于新的课时环节。
- 没有足够作品、答案或过程材料时使用“暂不评价”，不能用 0 分或 1 星代替。
- “暂不评价”原因分为缺少作品或答案、本节未安排或未观察到、不适用于当前任务、技术或数据问题、其他；整份评价可以全部暂不评价，但每项都必须说明原因。
- 平均星级只统计已评分项，教师还要看到评价提交人数和指标覆盖数，避免材料不足的学生被显示成低表现。
- 出勤、签到、积分和在线时长不能直接作为学科评价指标。
- 学生端只显示教师公开的评价要求和本人结果，不显示版本管理、内部分析或隐性分层信息。

完整规则见 [教师评价标准管理](evaluation_management.md)。

## 学生评价、积分与奖章

- 教师可查看本人任教班级学生的成绩、任务评价、核心素养相关材料、课堂激励积分和奖章，但这些数据必须分区展示。
- 教师加减分只能写不可变积分流水；修正通过反向流水完成，不能直接改历史总分。
- 缺勤、请假、离线、未投放任务和数据错误不自动扣分。
- 学生核心素养评价绑定具体学科、任务、评价版本和学习材料；普通点击、停留和积分不能直接生成素养分数。
- 教师自建或 AI 生成评价默认为本任务形成性反馈，不自动升级为校级核心素养认证。
- 教师端不提供积分排行榜作为分层依据；A/B/C、预测概率和风险原因继续仅在隐性分层审核工作台显示。
- 详细边界见 [学生评价、积分与奖章设计](student_evaluation_incentive_design.md)。
