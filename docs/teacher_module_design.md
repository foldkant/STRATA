...
- 课堂题支持题目级分层字段：`target_layer`、`use_layer_scores`、`layer_scores`。课堂启用分层模式后，学生端按学生当前 A/B/C 层级过滤题目并返回对应分值。
...
- 普通课堂：学生端显示当前投放环节下的全部题目。
- 分层课堂：`ClassroomSession.is_layered=true`，学生端只显示题目 `target_layer` 匹配自己当前层级的题目；若题目启用 `use_layer_scores`，分值按 `layer_scores.A/B/C` 返回。

分层模式是课堂级开关，不写死在课程或课时上。同一课时可以在普通课堂中使用，也可以在某个班级的分层课堂中使用。
...
- 底部：签到、随机点名、抢答、倒计时、广播、统一打开资源、锁定提交等工具。
