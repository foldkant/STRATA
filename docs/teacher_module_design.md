...
- 课程评价配置保存后发布不可变量规版本；课堂首次开启评价时冻结当时版本，后续编辑只影响新课堂。
- 教师修改师评时追加新提交版本，不覆盖旧评价。自评、互评、师评统一保留评价者、被评价学生、量规版本和前序修订关系。
...
- 教师导航新增 `/teacher/measurement-design`，用于设计任务蓝图和五星量规，不与普通课堂评价配置混用。
- 任务蓝图固定 `Claim -> Evidence -> Task -> Scoring -> Use`，发布前检查主张、证据和任务引用完整性。
- 量规条目按 P/S/R/C/D/E 模块组织；每项保存证据来源、不可观察条件、允许支持、反例、五级文字锚点、至少两份锚定样例和下一步形成性行动。
- 没有观察机会时记录 `NOT_ASSESSED`。出勤、按时率、完成率、积分、在线时长和服从性不能进入量规。
- 教师草案只允许 `local_formative`；校级共同测量和研究链接必须经过后续学校治理与验证流程。
- 发布版本不可修改；教师继续编辑草案并发布下一版本。学生端不显示版本治理、研究用途或内部层级信息。

完整规则见 [任务蓝图与量规版本](measurement_design.md)。

## 学生评价、积分与奖章

- 教师可查看本人任教班级学生的成绩、任务级五星量规、核心素养证据、课堂激励积分和奖章，但这些数据必须分区展示。
- 教师加减分只能写不可变积分流水；修正通过反向流水完成，不能直接改历史总分。
- 缺勤、请假、离线、未投放任务和数据错误不自动扣分。
- 学生核心素养评价绑定具体学科框架、任务、量规/测评版本和证据；普通点击、停留和积分不能直接生成素养分数。
