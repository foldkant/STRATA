...
type ModalFocusBinding = CloseHandler | false
...
function focusInitialElement(dialog: HTMLElement) {
  const preferred = dialog.querySelector<HTMLElement>('[data-modal-initial-focus]:not([disabled])')
  const target = preferred || focusableElements(dialog)[0] || dialog
  target.focus({ preventScroll: true })
}

export const vModalFocus: Directive<HTMLElement, ModalFocusBinding> = {
  ...
  updated(dialog, binding) {
    ...
    if (state && binding.value) state.close = binding.value
  },
