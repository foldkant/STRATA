...
const form = reactive<TestPayload>({
  ...
  randomize_question_order: false, randomize_option_order: false, common_question_set: ''
})

const availableCourses = computed(() => options.value?.courses.filter((item) => !form.subject || Number(item.subject) === Number(form.subject)) || [])
const availableCommonSets = computed(() => options.value?.common_question_sets.filter((item) => Number(item.subject) === Number(form.subject)) || [])
...
function questionRateLabel(value: number | null) {
  return value === null ? '数据不足' : `${value}%`
}

function questionBarWidth(value: number | null) {
  return `${value ?? 0}%`
}

function decimalMetric(value: number | null) {
  return value === null ? '数据不足' : value.toFixed(3)
}

function optionRate(count: number, answeredCount: number) {
  return answeredCount ? `${Math.round(count * 100 / answeredCount)}%` : '0%'
}
...
function resetForm() {
  editing.value = null
  errors.value = {}
  Object.assign(form, { title: '', subject: options.value?.subjects[0]?.id || '', course: '', common_question_set: '', class_ids: [], instruction: '', duration_minutes: 45, start_at: '', end_at: '', show_score_after_submit: false, randomize_question_order: false, randomize_option_order: false })
  paperItems.value = []
...
async function openEdit(row: TestAssessment) {
  try {
    ...
    Object.assign(form, {
      title: detail.title, subject: detail.subject.id, course: detail.course?.id || '', common_question_set: detail.common_question_set?.id || '',
...
async function saveInfoAndCompose() {
  ...
  try {
    ...
    const selectedSet = options.value?.common_question_sets.find((item) => item.id === Number(saved.common_question_set?.id || form.common_question_set))
    if (selectedSet) {
      selectedSet.items.filter((item) => item.required).forEach((setItem) => {
        const question = bankRows.value.find((item) => item.id === setItem.question_id)
        if (question && !inPaper(question.id)) paperItems.value.push({ question, score: question.default_score })
      })
    }
...
<template>
  <AppShell title="测试管理" eyebrow="教师工作台" :nav-items="navItems">
    ...
    <section class="panel assessment-management-panel">
      ...
      <div class="assessment-filter-bar compact"><select v-model="statusFilter" aria-label="按测试状态筛选" @change="load"><option value="">全部状态</option><option value="draft">草稿</option><option value="published">待开启</option><option value="open">进行中</option><option value="closed">已结束</option></select><button class="secondary-button" type="button" @click="load">刷新</button></div>
    ...
    <div v-if="editorOpen" class="modal-backdrop" @click.self="editorOpen = false"><section class="entity-modal assessment-builder-modal" role="dialog" aria-modal="true">
      ...
      <div v-if="editorStep === 'info'" class="assessment-modal-body">
        <div class="assessment-form-grid">
          ...
          <label><span>学科 <b class="required-mark" aria-hidden="true">*</b></span><select v-model="form.subject" :disabled="Boolean(editing)" required @change="form.common_question_set = ''"><option value="">请选择</option><option v-for="item in options?.subjects" :key="item.id" :value="item.id">{{ item.name }}</option></select><small v-if="errors.subject" class="field-error">{{ errors.subject[0] }}</small></label>
          <label><span>关联课程</span><select v-model="form.course"><option value="">不关联课程</option><option v-for="item in availableCourses" :key="item.id" :value="item.id">{{ item.title }}</option></select></label>
          <label><span>共同题集合</span><select v-model="form.common_question_set"><option value="">普通测试</option><option v-for="item in availableCommonSets" :key="item.id" :value="item.id">{{ item.title }} · v{{ item.version_no }} · {{ item.question_count }} 题</option></select><small v-if="errors.common_question_set" class="field-error">{{ errors.common_question_set[0] }}</small></label>
          ...
          <label><span>计划结束</span><input v-model="form.end_at" type="datetime-local" /><small v-if="errors.end_at" class="field-error">{{ errors.end_at[0] }}</small></label>
        </div>
        <section class="assessment-class-selector"><header><strong>安排班级 <b class="required-mark" aria-hidden="true">*</b></strong><MultiSelectActions :selected-count="form.class_ids.length" :total-count="options?.classes.length || 0" @select-all="form.class_ids = options?.classes.map((item) => item.id) || []" @clear="form.class_ids = []" /></header><div><label v-for="item in options?.classes" :key="item.id"><input type="checkbox" :checked="form.class_ids.includes(item.id)" @change="toggleClass(item.id, ($event.target as HTMLInputElement).checked)" /><span>{{ item.grade }} {{ item.name }}</span></label></div><small v-if="errors.class_ids" class="field-error">{{ errors.class_ids[0] }}</small></section>
    ...
    <div v-if="resultsOpen" class="modal-backdrop" @click.self="resultsOpen = false">
      <section class="entity-modal assessment-results-modal">
        <header class="modal-header">
          <div><h2>{{ results?.assessment.title || '测试成绩' }}</h2><p>完成情况、答卷批阅和逐题统计。</p></div>
          <div class="assessment-results-header-actions">
            <a v-if="results" class="secondary-button" :href="assessmentResultsExportUrl(results.assessment.id)">导出 XLSX</a>
            <button class="icon-button" type="button" aria-label="关闭" @click="resultsOpen = false">×</button>
          </div>
        </header>
        <div v-if="results" class="assessment-modal-body assessment-results-body">
          <section class="assessment-result-summary">
            <article><span>应考</span><strong>{{ results.summary.assigned_count }}</strong></article>
            <article><span>已提交</span><strong>{{ results.summary.submitted_count }}</strong></article>
            <article :class="{ attention: pendingGradeAttempts.length }"><span>待批阅</span><strong>{{ results.summary.pending_grade_count }}</strong></article>
            <article><span>平均分</span><strong>{{ results.summary.average_score }}</strong></article>
          </section>
          <section v-if="pendingGradeAttempts.length" class="assessment-pending-queue">
            <header><div><strong>待批阅答卷</strong><span>优先处理主观题，批阅后计入最终成绩</span></div><b>{{ pendingGradeAttempts.length }} 份</b></header>
            <div><button v-for="item in pendingGradeAttempts" :key="item.id" type="button" @click="openGrade(item)"><span><strong>{{ item.student.display_name }}</strong><small>{{ item.class_group.name }} · 已提交</small></span><em>开始批阅</em></button></div>
          </section>
          <div class="assessment-result-grid">
            <section>
              <h3>学生成绩</h3>
              <div class="assessment-table-wrap">
                <table class="assessment-table assessment-result-table">
                  <thead><tr><th>学生</th><th>班级</th><th>状态</th><th>得分</th><th>答卷</th></tr></thead>
                  <tbody><tr v-for="item in results.attempts" :key="item.id"><td>{{ item.student.display_name }}</td><td>{{ item.class_group.name }}</td><td><span class="status-pill" :class="item.status === 'submitted' ? 'status-published' : 'status-closed'">{{ item.status_label }}</span></td><td>{{ item.total_score ?? '-' }}</td><td><button class="assessment-row-review" type="button" @click="openGrade(item)">{{ item.status === 'submitted' ? '批阅' : '查看答卷' }}</button></td></tr></tbody>
                </table>
              </div>
            </section>
            <section>
              <h3>逐题表现</h3>
              <div class="assessment-stat-list">
                <article v-for="(item, index) in results.question_stats" :key="item.question.id">
                  <span>第 {{ index + 1 }} 题 · {{ item.question.question_type_label }}</span>
                  <strong>{{ item.question.stem }}</strong>
                  <div class="assessment-question-rate" :class="{ insufficient: item.data_status === 'insufficient' }">
                    <b>{{ questionRateLabel(item.correct_rate) }}</b>
                    <i v-if="item.data_status !== 'insufficient'"><em :style="{ width: questionBarWidth(item.correct_rate) }"></em></i>
                    <small>{{ item.correct_count }} / {{ item.answered_count }} 正确</small>
                  </div>
                  <dl class="assessment-question-metrics">
                    <div><dt>难度</dt><dd>{{ decimalMetric(item.difficulty) }}</dd></div>
                    <div><dt>区分度</dt><dd>{{ decimalMetric(item.discrimination) }}</dd></div>
                    <div><dt>有效样本</dt><dd>{{ item.sample_size }}</dd></div>
                  </dl>
                  <section v-if="item.option_distribution.length" class="assessment-option-distribution" aria-label="选项分布">
                    <div v-for="option in item.option_distribution" :key="option.option">
                      <span>{{ option.option }}</span>
                      <i><em :style="{ width: optionRate(option.count, item.answered_count) }"></em></i>
                      <small>{{ option.count }} 人</small>
                    </div>
                  </section>
                </article>
              </div>
        ...
        <p v-else class="empty">正在加载成绩</p>
        <footer class="modal-actions"><button class="secondary-button" type="button" @click="resultsOpen = false">关闭</button></footer>
      </section>
    </div>
